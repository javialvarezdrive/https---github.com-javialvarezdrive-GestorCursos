...
def main():
    ...
    if not is_authenticated:
        ...
        if not st.session_state.password_recovery:
            ...
            def process_login():
                ...
                try:
                    ...
                    # Redirigir a la página de Actividades después del login exitoso
                    st.session_state.redirect_to_activities = True
            ...
            if st.session_state.get('need_rerun', False):
                ...
                if st.session_state.get('redirect_to_activities', False):
                    st.session_state.redirect_to_activities = False
            ...
            st.title("👮‍♂️ Recuperación de Contraseña")
        ...
        # Usuario autenticado - Redirigir a la página de Actividades
        # Redirigir a la página de Actividades si no lo hemos hecho ya
        if not st.session_state.get('redirected_after_login', False):
            st.session_state.redirected_after_login = True
            st.switch_page("pages/3_Actividades.py")
        else:
            # Si ya estamos autenticados pero seguimos en esta página, mostrar un mensaje y un enlace
            st.success("Has iniciado sesión correctamente")
            if st.button("Ir a Actividades"):
                st.switch_page("pages/3_Actividades.py")
